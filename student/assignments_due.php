<?php
//assignments_due
//done by cross matching each student with their corrponding courses then lessons with assignments then with assignment due date then with current date
//SELECT * FROM `lessons` LEFT JOIN s_courses ON lessons.course = s_courses.course AND student = '1' WHERE lessons.assignment = 1 AND lessons.assignment_due > now()
...
for($i=0;$i<count($getAssignLesson); $i++){
    $complete = $getAssignLesson[$i]['completed'] ? " Completed" : " Incomplete";
    $date = date_create($getAssignLesson[$i]['assignment_due']);
    $complete .= " [".date_format($date,'Y/m/d')."]";

	$carddata = array($getAssignLesson[$i]['name'], "Assignment:".$complete, $getAssignLesson[$i]['description'],"lesson_page.php?cid=".$getAssignLesson[$i]['l_course']."&id=".$getAssignLesson[$i]['l_id'],'View');
